...
## テスト配布用の上書きアップデート対応 APK

実機でのテストを繰り返す際、新しい APK を入れるたびにアンインストールが必要だと、水分記録データや設定値が消えてしまいます。これを避けるため、テスト配布用に固定署名された debug APK を生成できるようにしています。

Android で APK を上書きアップデートするには、以下が必要です。

- `applicationId` が同じ
- 署名証明書が同じ
- `versionCode` が増えている

GitHub Actions の通常の debug ビルドは実行環境ごとに異なる debug keystore で署名されるため、別署名アプリ扱いとなり上書きできない場合があります。そこで、固定の署名鍵で署名する専用ワークフロー `Android Signed Debug APK` を用意しました。

### ワークフロー

- 既存の `Android Debug APK`（`android-debug-apk.yml`）はそのまま維持します。
- 固定署名版は `Android Signed Debug APK`（`android-signed-debug-apk.yml`）で、手動実行（`workflow_dispatch`）専用です。
- ビルド時に `-PVERSION_CODE=${{ github.run_number }}` を渡し、実行ごとに `versionCode` が増えるようにしています。
- 生成される APK は `mizunomi-debug-v<run_number>.apk` という名前で artifact としてアップロードされます。

### 必要な GitHub Secrets

固定署名版を使うには、リポジトリの **Settings → Secrets and variables → Actions** に以下の Secrets を設定してください。

| Secret 名 | 内容 |
| --- | --- |
| `MIZUNOMI_UPLOAD_KEYSTORE_BASE64` | テスト配布用 keystore を base64 エンコードした文字列 |
| `MIZUNOMI_UPLOAD_KEYSTORE_PASSWORD` | keystore のパスワード |
| `MIZUNOMI_UPLOAD_KEY_ALIAS` | 署名鍵のエイリアス |
| `MIZUNOMI_UPLOAD_KEY_PASSWORD` | 署名鍵のパスワード |

これはあくまでテスト配布用の署名であり、正式リリース用（Google Play / Play App Signing）の署名とは分けて扱います。

### keystore の作り方と base64 化

テスト配布用 keystore はローカルで作成し、リポジトリにはコミットしません。

```bash
# テスト配布用 keystore を作成（パスワード・エイリアスは任意の値に置き換える）
keytool -genkeypair -v \
  -keystore upload-keystore.jks \
  -alias mizunomi-upload \
  -keyalg RSA -keysize 2048 -validity 10000

# base64 化して MIZUNOMI_UPLOAD_KEYSTORE_BASE64 に貼り付ける
base64 -w 0 upload-keystore.jks
```

生成した `upload-keystore.jks` や各パスワードは **リポジトリにコミットしないでください**。GitHub Secrets にのみ保存します。

### ローカルビルドへの影響

Secrets（および対応する環境変数）が未設定の環境では、`app/build.gradle.kts` の固定署名は無効化され、Android 標準の debug keystore で署名されます。そのため `./gradlew assembleDebug` は従来どおり動作します。

ローカルで固定署名を試したい場合は、以下の環境変数を設定してビルドします。

```bash
export MIZUNOMI_UPLOAD_KEYSTORE_PATH=/absolute/path/to/upload-keystore.jks
export MIZUNOMI_UPLOAD_KEYSTORE_PASSWORD=...
export MIZUNOMI_UPLOAD_KEY_ALIAS=mizunomi-upload
export MIZUNOMI_UPLOAD_KEY_PASSWORD=...
./gradlew assembleDebug -PVERSION_CODE=2
```
