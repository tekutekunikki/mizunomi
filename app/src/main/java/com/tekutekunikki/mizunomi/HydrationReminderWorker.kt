...
class HydrationReminderWorker(
    ...
    override suspend fun doWork(): Result {
        ...
        val todayTotalMl = repository.getTotalAmountForDay(LocalDate.now())
        ...
        if (
            ...
            !wasAlreadyNotified(checkHour)
        ) {
            HydrationReminderNotifications.showHydrationReminder(applicationContext, checkHour)
            markNotified(checkHour)
        }

        return Result.success()
    }

    private fun wasAlreadyNotified(checkHour: Int): Boolean =
        preferences().getBoolean(notificationKey(checkHour), false)

    private fun markNotified(checkHour: Int) {
        preferences()
            .edit()
            .putBoolean(notificationKey(checkHour), true)
            .apply()
    }

    private fun notificationKey(checkHour: Int): String {
        val today = LocalDate.now().format(DateTimeFormatter.BASIC_ISO_DATE)
        return "$today-$checkHour"
    }
